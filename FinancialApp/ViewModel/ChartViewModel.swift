...
protocol ChartViewModelProtocol: ObservableObject {
    var chartState: RequestState { get }
    var selectedChart: ChartViewData? { get }
    var selectedRange: ChartRange { get }
    var selectedX: (any Plottable)? { get set }
    var selectedXRuleMark: (value: Int, text: String)? { get }
    var foregroundMarkColor: Color { get }
    var selectedXDateText: String { get }
    var selectedXOpacity: Double { get }
...
class ChartViewModel: ObservableObject {
    ...
    var selectedXRuleMark: (value: Int, text: String)? {
        ...
        return (selectedX, selectedChart.items[selectedX].value.roundedString)
    }
    
    var foregroundMarkColor: Color {
        (selectedX != nil) ? .cyan : (selectedChart?.lineColor ?? .cyan)
    }
    ...
    var selectedXDateText: String {
        guard let selectedX = selectedX as? Int, let selectedChart else { return "" }
        ...
        let item = selectedChart.items[selectedX]
    ...
    init(ticker: Ticker, dataService: DataServiceProtocol = XCAStocksAPI()) {
        self.ticker = ticker
...
extension ChartViewModel: ChartViewModelProtocol {
    func fetchData() async {
        do {
            chartState = .loading
            let rangeType = self.selectedRange
            let chartData = try await dataService.fetchChartData(tickerSymbol: ticker.symbol, range: rangeType)
            
            guard rangeType == self.selectedRange else { return }
            if let chartData {
                selectedChart = transformChartViewData(chartData)
                chartState = .success
            } else {
                chartState = .empty
            }
        } catch {
            chartState = .failure(error)
        }
    }
}
